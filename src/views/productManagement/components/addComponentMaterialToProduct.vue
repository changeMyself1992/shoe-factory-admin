<template>
  <div id="add-component-material-to-product" class="add-component-material-to-product">
    <el-button
      v-if="modeComponents!=='detail'"
      type="primary"
      size="small"
      :loading="loading"
      @click="intelligentMatchingBtnClick()"
    >部位物料信息智能填充</el-button>
    <el-table id="productLocationAndMaterialInfoSheet" class="table" :data="部位详情" border>
      <el-table-column type="index" width="50" align="center" />

      <el-table-column v-if="modeComponents!=='detail'" label="操作" align="center" width="180">
        <template slot-scope="scope_e">
          <div class="opetion-box">
            <el-button
              type="danger"
              size="mini"
              :loading="btnLoading&&curIndex===scope_e.$index"
              @click="clearPart(scope_e.$index)"
            >清空</el-button>
            <i class="el-icon-remove-outline remove" @click="removePart(scope_e.$index)" />
            <i
              class="el-icon-circle-plus-outline continue-add"
              @click="continueAdd(scope_e.$index)"
            />
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="使用物料.参考市场单价" label="参考市场单价" align="center" width="150">
        <template slot-scope="scope_b">
          <el-input
      ...
      <el-table-column prop="使用物料.备注" label="备注" align="center" width="150">
        <template slot-scope="scope_b">
          <el-input
    ...
    />
    <el-dialog title="相似产品选择" :visible.sync="dialogVisible" width="70%" center>
      <div class="info-box">
        <strong>打钩生效</strong>
        <el-form ref="form" :model="form" label-position="left">
          ...
          <div v-loading="whetherLabelInputProcessingIsTakingPlace" class="row-1">
            <el-form-item
              ...
              :label="tag_name"
            >
              <el-checkbox v-model="form.tagsCheckbox[tag_name]" @change="checkboxChange" />
              <el-autocomplete
                ...
                placeholder="自动补全"
                :disabled="!form.tagsCheckbox[tag_name]"
                @blur="handleBlur"
              />
            </el-form-item>
          </div>
      ...
      <el-table :data="arrayOfSimilarProducts" border>
        <el-table-column label="选择" width="70" align="center">
          <template slot-scope="scope">
            <el-radio v-model="curSelectIndex" class="radio" :label="scope.$index">&nbsp;</el-radio>
          </template>
        </el-table-column>
        <el-table-column prop="产品编号" label="产品编号" align="center" width="150" />
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.tags[name] }}</span>
          </template>
        </el-table-column>

        <el-table-column prop="部位详情" align="center">
          <template slot="header">
            <el-button type="primary" size="small" @click.native="partArrowClick()">
          ...
          <el-table-column
            v-for="(item, index) in 部位列表"
            :key="index"
            align="center"
            :width="whetherToExpandLocationDetails ? '100' :''"
          >
            <template slot="header">
              <!--自定义表头-->
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="determineBtnClick">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 相似产品选择确认关闭
    determineBtnClick() {
      if (this.arrayOfSimilarProducts.length > 0) {
        this.curSelectedProduct = this.arrayOfSimilarProducts[this.curSelectIndex]
        this.部位详情 = this.curSelectedProduct['部位详情']
      }
