  ...
  <div class="invoice">
    <el-form ref="form" class="form" :model="form" label-position="left">
      ...
      <el-card class="box-card">
        ...
        <el-table ref="table" class="table" :data="form.送货单" border>
          <el-table-column align="center" prop="名称及规格" label="名称及规格" width="200p">
          ...
          <el-table-column align="center" prop="货号" label="货号">
            <template slot-scope="scope">
              <el-input v-model="scope.row.货号" />
            </template>
          </el-table-column>
          <el-table-column align="center" prop="单位" label="单位">
            <template slot-scope="scope">
              <el-input v-model="scope.row.单位" />
            </template>
          </el-table-column>
...
export default {
    ...
    'form.供应商名称': function(newVal, oldVal) {
      for (let i = 0; i < this.factoryInformationList.length; i++) {
        ...
        if (element.名称 === newVal) {
          ...
          var 送货单 = []
          送货单.push(deepClone(this.$store.getters['空的供应商送货单元素']))
          this.form['送货单'] = 送货单
          return
        }
      }
  ...
  methods: {
    // 初始化form
    initializationForm() {
      var form = deepClone(this.$store.getters['空的供应商送货单'])
      ...
      form['送货单'].push(
        deepClone(this.$store.getters['空的供应商送货单元素'])
      )
    ...
    handleSelect(item, index) {
      ...
      var invoice = this.form['送货单'][index]
      var i = this.nameAndSpecificationList.findIndex(item => {
        return item['名称及规格'] === invoice['名称及规格']
      })
      if (i !== -1) {
        // 如果找到了
        this.form['送货单'][index]['单价'] = this.nameAndSpecificationList[i][
          '单价'
        ]
        this.form['送货单'][index]['单位'] = this.nameAndSpecificationList[i][
          '单位'
        ]
        this.form['送货单'][index]['备注'] = this.nameAndSpecificationList[i][
          '备注'
        ]
        this.form['送货单'][index]['货号'] = this.nameAndSpecificationList[i][
          '货号'
        ]
      }
      return
    ...
    onBlurElementsInfo(mode, index) {
      if (mode === '数量') {
        this.form['送货单'][index]['数量'] = isEmpty(
          this.form['送货单'][index]['数量']
        )
          ? 0
          : Number(parseFloat(this.form['送货单'][index]['数量']).toFixed(2))
        // 如果是数量得话 总价格=数量*单价
        this.form['送货单'][index]['总价格'] = Number(
          parseFloat(
            this.form['送货单'][index]['数量'] *
              this.form['送货单'][index]['单价']
          ).toFixed(2)
        )
      } else if (mode === '单价') {
        this.form['送货单'][index]['单价'] = isEmpty(
          this.form['送货单'][index]['单价']
        )
          ? 0
          : Number(parseFloat(this.form['送货单'][index]['单价']).toFixed(2))
        // 如果是单价得话 总价格=数量*单价
        this.form['送货单'][index]['总价格'] = Number(
          parseFloat(
            this.form['送货单'][index]['单价'] *
        ...
        this.form['送货单'][index]['总价格'] = isEmpty(
          this.form['送货单'][index]['总价格']
        )
          ? 0
          : Number(parseFloat(this.form['送货单'][index]['总价格']).toFixed(2))
        // 如果单价不为0 那么计算数量
        if (this.form['送货单'][index]['单价'] !== 0) {
          this.form['送货单'][index]['数量'] = Number(
            parseFloat(
              this.form['送货单'][index]['总价格'] /
                this.form['送货单'][index]['单价']
            ).toFixed(2)
          )
        } else if (this.form['送货单'][index]['数量'] !== 0) {
          // 如果数量不为0 那么计算单价
          this.form['送货单'][index]['单价'] = Number(
            parseFloat(
              this.form['送货单'][index]['总价格'] /
                this.form['送货单'][index]['数量']
            ).toFixed(2)
    ...
    removeElementsFromList(index) {
      if (this.form['送货单'].length <= 1) {
        for (const key in this.form['送货单'][0]) {
          this.form['送货单'][0][key] = this.$store.getters[
            '空的供应商送货单元素'
          ][key]
        }
        return
      }
      this.form['送货单'].splice(index, 1)
    },
    // 继续添加货物
    addElementsOfGoods() {
      // 等待一下先让失去焦点的逻辑处理完
      setTimeout(() => {
        ...
        var emptyElement = deepClone(
          this.$store.getters['空的供应商送货单元素']
        )
        this.form['送货单'].push(emptyElement)
      }, 100)
    },

    // 继续添加货物的时候，判断form.送货单
    continueToAddGoodsJudgmentCriteria() {
      for (let r = 0; r < this.form['送货单'].length; r++) {
        const element_wai = this.form['送货单'][r]
        if (isEmpty(element_wai.名称及规格)) {
          this.$message({
            message: `送货单第${r + 1}行，名称及规格是必填项！`,
            type: 'warning'
          })
          return false
        }
        ...
        if (isEmpty(element_wai.数量) && isEmpty(element_wai.单价)) {
          this.$message({
            message: `送货单第${r + 1}行，数量和单价至少要输入1项！`,
            type: 'warning'
          })
          return false
        }
        if (isEmpty(element_wai.总价格)) {
          this.$message({
            message: `送货单第${r + 1}行，总价格是必填项！`,
            type: 'warning'
          })
          return false
        }
        for (let c = r + 1; c < this.form['送货单'].length; c++) {
          // 如果找到重复的名称或者是完全一样的unique_id，那么也不合规
          const element_nei = this.form['送货单'][c]
          if (element_nei.名称及规格 === element_wai.名称及规格) {
            this.$message({
              message: '送货单中不允许出现重复的名称及规格',
              type: 'warning'
            })
            return false
          }
    ...
    formSubmit(e) {
      ...
      this.$nextTick(async() => {
        ...
        if (isEmpty(this.form['供应商名称']) || isEmpty(this.form['供应商unique_id'])) {
          this.loading = false
          this.$message({
            message: '不存在的供应商无法提交货单！',
            type: 'warning'
          })
          return
        }
        ...
        try {
          var res = await add_supplier_transaction(paramter)
          this.formReset()
